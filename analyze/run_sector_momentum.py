...
try:
    ...
    from sector_momentum_backtester import SectorMomentumBacktester
except ImportError as e:
    print(f"❌ 모듈 import 오류: {e}")
    print("📝 sector_momentum_analyzer.py와 sector_momentum_backtester.py가 필요합니다.")
    sys.exit(1)
...
def main():
    """메인 실행 함수"""
    # 명령행 인자 파싱
    parser = argparse.ArgumentParser(description='업종 모멘텀 전략 실행')
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('sector_momentum.log', encoding='utf-8')
        ]
    )
    ...
    try:
        ...
        print(f"  • 백테스트: {'예' if args.backtest else '아니오'}")
        if args.backtest:
            print(f"  • 보유 기간: {args.holding_days}영업일")
        print("=" * 70)
        ...
        # 추천 종목 추출
        logger.info(f"\n📈 {args.consecutive_days}일 연속 상승 업종 분석 시작...")
        ...
        if not recommendations:
            ...
            print(f"\n📭 현재 {args.consecutive_days}일 연속 상승한 업종이 없습니다.")
            return 0
        ...
        if args.backtest:
            logger.info("\n📊 백테스트 시작...")
            backtester = SectorMomentumBacktester(api_client, analyzer)
            
            stats = backtester.backtest_current_recommendations(
                recommendations,
                holding_days=args.holding_days
            )
            
            if stats['total_trades'] > 0:
                backtest_report = backtester.format_backtest_report(stats)
        ...
        # JSON 파일 저장
        if args.save_json:
            output_file = f"sector_momentum_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
            ...
            output_data = {
                ...
                'parameters': {
                    ...
                    'holding_days': args.holding_days if args.backtest else None
                },
                'recommendations': recommendations,
                'backtest_stats': stats if args.backtest else None
            }
